...
function buildValidateSystem(tagOptions: DestinationTagOptions): string {
  return `You are a classification QA agent. Score a per-idea classification result 0-10 and return JSON:
- "score": integer 0-10
...
- Tags must come ONLY from the allowed list for the chosen destination:
  - projects: ${JSON.stringify(tagOptions.projects)}
  - journal: ${JSON.stringify(tagOptions.journal)}
  - knowledge: ${JSON.stringify(tagOptions.knowledge)}
...
export async function validateClassification(
  ...
  tagOptions: DestinationTagOptions
